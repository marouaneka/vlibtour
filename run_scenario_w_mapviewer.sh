...
if [ -n "$GLASSFISH_HOME" ]; then
    ...
    asadmin deploy vlibtour-tour-management-system/vlibtour-tour-management-bean/target/vlibtour-tour-management-bean.jar
fi
...
if [ -n "$RABBITMQ_MNESIA_BASE" ]; then
    ...
    echo "Problem with the configuration of RabbitMQ"
    exit 1
fi
