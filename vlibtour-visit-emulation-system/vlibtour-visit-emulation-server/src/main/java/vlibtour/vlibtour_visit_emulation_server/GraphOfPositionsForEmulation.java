 ...
 * <p>
 * This is only for writing scenarios with the emulation of locations, i.e.
 * using the VLibTour Visit Emulation System.
 * <p>
 ...
 * of the management of the visits! For instance, the VLibTour Visit Emulation
 * Server that is accessed by REST calls must not be bypassed.
 * <p>
 * A limitation of the current implementation is that there is only one graph of
 * positions.
 * <p>
 * The first part of the class is the definition of the graph with some utility
 * methods: e.g. {@link #addEdge(Map, Position, Position)} or
 * {@link #computePathsFromDepartureToDestination(Map, Object, Object)}.
 * <p>
 * The second part of the class is the definition of paths with some utility
 * methods: {@link #stepInCurrentPath(String)}, etc.
 * <p>
...
public final class GraphOfPositionsForEmulation {
	...
	/**
	 * private constructor of the utility class to avoid instantiation.
	 */
	private GraphOfPositionsForEmulation() {
	}

	// Methods that are part of the API and that are accessed through delegation.
	...
	/**
	 * sets the possible paths to the next positions, chose one of the paths, and
	 * set the current position to the first position of the chosen path. The
	 * arguments are the user and the destination position, e.g. the next POI in the
	 * visit.
	 * 
	 * @param user        the identifier of the user.
	 * @param destination the destination position.
	...
	/**
	 * sets the current position of the given user at the beginning of the visit.
	 * The method must be called only once per user.
	 * 
	 * @param user     the identifier of the user.
