...
Copyright (C) 2017_2020 Télécom SudParis
 ...
 * {@link com.grum.geocalc.Point} by delegation for providing utility methods.
 * <p>
...
public class GPSPosition {
	...
	/**
	 * computes the distance from this location to a target location in meters (as
	 * the crow flies).
	 * 
	 * @param target the target location to calculate the distance from.
	 * @return the distance in meter.
	 */

	public double distanceFrom(final GPSPosition target) {
		return EarthCalc.getDistance(new Point(new DegreeCoordinate(latitude), new DegreeCoordinate(longitude)),
				new Point(new DegreeCoordinate(target.getLatitude()), new DegreeCoordinate(target.getLongitude())));
	}
